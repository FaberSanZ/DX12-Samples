// AccelerationStructures.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
#include <Windows.h>
#include <d3d12.h>
#include <dxgi1_4.h>
...
public:
    ...
    void Initialize(HWND handle, uint32_t w, uint32_t h)
    {
        ...
        m_SrvUavHeap = createDescriptorHeap(pDevice, 2, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, true);
    ...
    AccelerationStructureBuffers createBottomLevelAS(ID3D12Device5* pDevice, ID3D12GraphicsCommandList4* pCmdList, ID3D12Resource* pVB)
    {
        ...
        geomDesc.Triangles.VertexBuffer.StrideInBytes = sizeof(float) * 3; // Only position, no normals or UVs
    ...
    void createShaderResourceUAV()
    {
        ...
        uavCpuHandle.ptr += 1 * descriptorSize;
    ...
    void Loop()
    {
        ...
        D3D12_GPU_DESCRIPTOR_HANDLE srvHandle = m_SrvUavHeap->GetGPUDescriptorHandleForHeapStart();
        srvHandle.ptr += 0 * pDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV); // indice 0 for SRV (tlas)
        mpCmdList->SetComputeRootDescriptorTable(0, srvHandle); // Set the SRV for the TLAS in the root signature. The shader will be able to access it using t0

        D3D12_GPU_DESCRIPTOR_HANDLE uavHandle = m_SrvUavHeap->GetGPUDescriptorHandleForHeapStart();
        uavHandle.ptr += pDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV); // indice 1 for UAV (output)
        mpCmdList->SetComputeRootDescriptorTable(1, uavHandle); // Set the UAV for the output resource in the root signature. The shader will be able to access it using u0
...
int main()
{
    uint32_t width = 1280;
    uint32_t height = 820;
    const wchar_t title[] = L"DirectX Ray Tracing AccelerationStructures";
